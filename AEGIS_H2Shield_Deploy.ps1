...
function Invoke-Hardening {
    ...
    # If a worker balloons (the exact failure mode of an HPACK bomb), recycle it
    # rather than letting it drag the box into swap. 1.5 GB private-bytes cap.
    if (-not $WhatIfOnly) {
        try {
            Import-Module WebAdministration -ErrorAction SilentlyContinue
        ...
        $actions.Add('[plan] Set every AppPool privateMemory recycle cap to 1.5 GB')
    }

    # --- (d) Restart HTTP.SYS-dependent services so HTTP/2 change takes effect ---
    if (-not $WhatIfOnly) {
        try {
            Stop-Service W3SVC -Force -ErrorAction SilentlyContinue
            Restart-Service HTTP -Force -ErrorAction SilentlyContinue
            Start-Service W3SVC -ErrorAction SilentlyContinue
            $actions.Add('Restarted HTTP.SYS + W3SVC to apply HTTP/2 disable.')
        } catch { $actions.Add("Service restart issue: $($_.Exception.Message)") }
    } else {
        $actions.Add('[plan] Restart HTTP.SYS + W3SVC')
    }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="AEGIS H2-SHIELD — CVE-2026-49975 Defense" Height="640" Width="980"
...
function Start-AuditAsync {
    ...
    foreach ($b in @($BtnAudit,$BtnHarden,$BtnTask)) { $b.IsEnabled = $false }
        ...
        try {
            ...
            if($svc){
                ...
                if($DoHarden){
                    ...
                    $actions+='HTTP/2 disabled at HTTP.SYS; header limits clamped.'
                    try{Import-Module WebAdministration -ErrorAction SilentlyContinue
                        Get-ChildItem IIS:\AppPools -ErrorAction SilentlyContinue|ForEach-Object{
                            ...
                            $actions+="AppPool '$($_.Name)': 1.5GB memory recycle cap."}}catch{}
                    try{Stop-Service W3SVC -Force -ErrorAction SilentlyContinue;Restart-Service HTTP -Force -ErrorAction SilentlyContinue;Start-Service W3SVC -ErrorAction SilentlyContinue
                        $actions+='Restarted HTTP.SYS + W3SVC.'}catch{$actions+="Restart issue: $($_.Exception.Message)"}
                    $drift=$false
                }
...
$timer.Add_Tick({
    ...
    if (-not $s.Running -and ($s.Report -or $s.Error)) {
        ...
        foreach ($b in @($BtnAudit,$BtnHarden,$BtnTask)) { $b.IsEnabled = $true }
...
$BtnHarden.Add_Click({
    $r=[System.Windows.MessageBox]::Show(
        'This will DISABLE HTTP/2 in HTTP.SYS, clamp header limits, set AppPool memory caps, and restart W3SVC/HTTP. Proceed?',
